...
def find_sensitive_text_image(image) -> list[tuple[int, int, int, int]]:
    """
    1. Capture the screen image.
    2. Send the image directly to Gemini for sensitive information detection.
    3. Return bounding boxes for sensitive regions.
    """
    try:
        ...
        prompt = "Detect sensitive information in the provided image. Return bounding boxes for sensitive regions."
        ...
        return []  # Return an empty list if Gemini fails
...
with mss() as sct:
    ...
    try:
        while True:
            ...
            # ── Detect sensitive text using Gemini ─────────────
            if frame_i % args.interval == 0:
                txt = find_sensitive_text_image(raw)
                txt = [expand_box(b, args.pad, W, H) for b in txt]
                text_cache.update(txt)
